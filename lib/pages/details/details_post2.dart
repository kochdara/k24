...
class _TestingPage4State extends ConsumerState<DetailsPost> {
    ...
    if(dataDetails.data != null) {
      ...
      if (dataDetails.data?.location != null) {
        futureAwait(duration: 10, () {
          ...
          var date = stringToString(date: '${dataDetails.data?.renew_date}', format: 'dd, MMM yyyy');
                        ...
                        data: (data) {
                          ...
                          if (datum?.location != null) {
                            futureAwait(duration: 10, () {
                              ...
                              var date = stringToString(date: '${datum?.renew_date}', format: 'dd, MMM yyyy');
                              if(date != null) ref.read(location.notifier).state += ' • $date';
  ...
  Widget imageList(thumbnail, listImg) {
    return InkWell(
      onTap: () {
  ...
  Widget contactCard({String tell = '', String url = '', }) {
    var sub3 = tell;
...
class _PreviewImageState extends ConsumerState<PreviewImage> {
  ...
  void setupPage() async {
    ...
    await futureAwait(duration: 50, () { ref.read(loadProvider.notifier).state = false; });
  }
