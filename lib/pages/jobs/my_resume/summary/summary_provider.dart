...
class GetSummary extends _$GetSummary {
  ...
  FutureOr fetchHome() async {
    try {
      ...
      String subs = 'me/resume/summary?lang=$lang';
...
class SummaryApiService {
  ...
  Future submitSummary(WidgetRef ref , Map<String, dynamic> data, ) async {
    dialogBuilder(ref.context);
    final tokens = ref.watch(usersProvider);
    final formData = FormData.fromMap(data);
    final subs = 'me/resume/summary?lang=$lang';
    try {
      ...
      if (e.response != null) {
        ...
        if (response?.statusCode == 401) {
          ...
          return await submitSummary(ref, data); // Retry the request after refreshing the token
        } else {
