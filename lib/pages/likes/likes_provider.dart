...
@riverpod
class GetTotalLikes extends _$GetTotalLikes {
  ...
  @override
  FutureOr<List<LikesDatum?>?> build(WidgetRef context, String sort) async {
    sorts = sort;
    if(listData.isEmpty) await fetchLikes(sorts);
    return listData;
  }
  ...
  Future<void> refresh() async {
    ...
    await fetchLikes(sorts);
    state = AsyncData(listData);
  }

  Future<void> fetchLikes(String? sort) async {
    try {
      final tokens = ref.watch(usersProvider);
      String subs = 'me?lang=$lang&fields=$fields&offset=$offset&sort=$sorts';
      final res = await dio.get('$likeUrl/$subs', options: Options(headers: {
      ...
      if (response?.statusCode == 401) {
        ...
        await fetchLikes(sort); // Retry the request after refreshing the token
        return;
      }
...
class LikesApiService {
  ...
  Future<MessageLogin> submitRemoveLike(WidgetRef ref , String? idLikes,) async {
    final tokens = ref.watch(usersProvider);

    final subs = 'me/$idLikes?lang=$lang';
    try {
      ...
      if (e.response != null) {
        ...
        if (response?.statusCode == 401) {
          ...
          return await submitRemoveLike(ref, idLikes); // Retry the request after refreshing the token
        }
      }
    } catch (e) {
      print(e);
